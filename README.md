# Temporary Name: Ziqqurat Like

My little side project that I’d like to create a 2D Top-down shooter template based on.
 ...
 * open terminal and locate game folder `cd C:\Users\"user_name"\Downloads\pixel-ziqqurat-like` replace `user_name` with your `name` and press enter
 * then run it with `cargo run` or build executable with `cargo run --release`
